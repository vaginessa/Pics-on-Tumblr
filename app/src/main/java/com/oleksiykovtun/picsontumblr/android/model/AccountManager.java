...
/**
 * tumblr.com API and user account manager // todo split into adapter and model
 */
public class AccountManager {
    ...
    public static void finishAuthorization(final String verifierString) {
        new AsyncTask<String, String, String>() {
            ...
            protected String doInBackground(String... params) {
                ...
                MainActivity.get().writePreferences(TAG_USER_KEY, accessToken.getToken());
                MainActivity.get().writePreferences(TAG_USER_SECRET, accessToken.getSecret());
                return "";
            }
    ...
    public static void revokeAuthorization() {
        MainActivity.get().writePreferences(TAG_USER_KEY, "");
        MainActivity.get().writePreferences(TAG_USER_SECRET, "");
        authenticationRequestToken = null;
    }

    public static boolean isClientAuthorized() {
        return ! MainActivity.get().readPreferences(TAG_USER_KEY, "").isEmpty();
    }

    public static JumblrClient getAccountClient() {
        JumblrClient client = new JumblrClient(API_KEY, API_SECRET);
        client.setToken(MainActivity.get().readPreferences(TAG_USER_KEY, ""),
                MainActivity.get().readPreferences(TAG_USER_SECRET, ""));
        return client;
    }
